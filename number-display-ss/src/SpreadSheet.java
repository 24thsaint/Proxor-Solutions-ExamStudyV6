...
public class SpreadSheet extends JFrame {
    ...
    public String evaluateToken(String tok, int depth) {
        if (tok.length() >= 2 && tok.charAt(0) >= 'A' && 
            ...
            if (row >= 0 && row < maxRows) {
                ...
                return cells[row][col].actualValue;
            }
        }
        return null;
    }
    ...
    public String truncateValue(String value) {
    	Scanner doubleScanner = new Scanner(value);
    	if (doubleScanner.hasNextDouble() && value.length() > 8) {
    		double parsedValue = doubleScanner.nextDouble();
    		if (parsedValue % 1 == 0 || parsedValue - 1 < 0) { // parsedValue is a whole number
        		return String.format("%.3g", parsedValue);
        	} else {	// parsedValue contains a decimal
        		return String.format("%.7g", parsedValue);
        	}
    	}
    	return value;
    }
    ...
    public void evaluate(int r, int c, int depth) {
        ...
        if (formula.length() > 0 && formula.charAt(0) == '=') {
            try {
                if (depth <= maxRows * maxCols) {
                    ...
                    if (tokens.hasMoreTokens() && 
                        ...
                        if (val != null) {
                        	cells[r][c].actualValue = val;
                        	val = this.truncateValue(val);
        	...
        	cells[r][c].actualValue = formula;
        	formula = this.truncateValue(formula);
