...
public class SpreadSheet extends JFrame {
    ...
    public String calculateAverage(String start, String end, int depth) {
    	int startIndex = 0;
    	int endIndex = 0;
    	int constant = 0;
    	double average = 0;
    	...
    	if (start.charAt(0) == end.charAt(0)) {
    		...
    		constant = start.charAt(1);
    		startIndex = start.charAt(0);
    		endIndex = end.charAt(0);
    		int dataCount = (endIndex - startIndex) + 1;
    		for (int i = startIndex; i <= endIndex; i++) {
    			String concatenatedCellData = (char) i + "" + (char) constant;
    			average += Double.parseDouble((this.evaluateToken(concatenatedCellData, depth)));
    		}
    		return String.valueOf(average / dataCount);
    	} else {
    ...
    public String parseFormula(StringTokenizer tokens, int depth) 
        ...
        if (tokens.hasMoreTokens()) {
            ...
            if (tok.equals("AVG")) {
            	...
            	String separator = tokens.nextToken();
            	String end = tokens.nextToken();
            	String closeParens = tokens.nextToken();
            	
            	return this.calculateAverage(start, end, depth);
            }
